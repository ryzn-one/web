...
import { Card, Label, Btn, Monogram, Field, XPPill, Ring, Bar, QR, BadgeGlyph, BadgeTile, Heatmap, HeaderRow, Glyph, TypingDots, BrandLogo, BrandIcon } from "./ui.jsx";
...
export const Splash = ({ onEnter, isDesktop }) => (
  <div onClick={isDesktop ? undefined : onEnter} style={{
    ...
    <div style={{ position: "absolute", inset: 0, backgroundImage: `url(${Brand.motif.diamonds})`, backgroundSize: "420px", opacity: 0.14 }} />
    ...
    <div className="splash-in" style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
      <BrandIcon size={72} style={{ boxShadow: "0 18px 40px rgba(20,10,60,.35)", marginBottom: 26 }} />
      <BrandLogo variant="stacked" color="white" height={96} />
      {isDesktop && (
...
export const RoleSelect = ({ onPick }) => (
  <div style={{ padding: "0 24px", height: "100%", display: "flex", flexDirection: "column" }}>
    <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
      <BrandLogo variant="horizontal" color="purple" height={32} />
...
export const Register = ({ role, go, onDone }) => {
  ...
  const [pw, setPw] = useState(role === "mentee" ? "RiseWithAlex26!" : "MentorJordan26!");
...
const CONSUMER_CREDS = {
  mentee: { email: "alex@ryzn.one", password: "RiseWithAlex26!" },
  mentor: { email: "jordan@harbourline.com", password: "MentorJordan26!" },
};

export const Login = ({ go, onDone, role }) => {
  const demo = CONSUMER_CREDS[role] || CONSUMER_CREDS.mentee;
  const [show, setShow] = useState(false);
  ...
  const submit = () => {
    ...
    const ok = Object.values(CONSUMER_CREDS).some(c => c.email === e && c.password === pw);
    if (!ok) { setErr("Email or password doesn't match."); return; }
    onDone();
  };
  return (
    <div style={{ padding: "0 24px" }}>
      ...
      <Field label="Password" type={show ? "text" : "password"} value={pw} onChange={e => { setPw(e.target.value); setErr(""); }}
        right={<button onClick={() => setShow(s => !s)} style={{ background: "none", border: "none", cursor: "pointer", padding: 4 }}>{show ? <EyeOff size={16} color={C.gray} /> : <Eye size={16} color={C.gray} />}</button>} />
      {err && <div style={{ fontSize: 12.5, color: C.coral, fontWeight: 600, marginTop: 8 }}>{err}</div>}
      ...
      <Btn style={{ marginTop: 18 }} onClick={submit}>Sign in</Btn>
      <div style={{ marginTop: 14, background: C.surface, border: `1px solid ${C.line}`, borderRadius: 12, padding: "10px 12px" }}>
        <div style={{ fontFamily: F.mono, fontSize: 8.5, color: C.gray, letterSpacing: 0.8 }}>DEMO CREDENTIALS</div>
        <div style={{ fontSize: 12, color: C.ink, marginTop: 6, lineHeight: 1.5 }}>
          <div><b>Mentee</b> · alex@ryzn.one · RiseWithAlex26!</div>
          <div><b>Mentor</b> · jordan@harbourline.com · MentorJordan26!</div>
        </div>
      </div>
